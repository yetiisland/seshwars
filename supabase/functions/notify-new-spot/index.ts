...
serve(async (req) => {
  try {
    ...
    // ---------------------------------------------------------------
    // Branch 1: user-submitted content report (Nudity / Gore)
    // ---------------------------------------------------------------
...
<body style="margin:0;padding:0;background:#FDF8F0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:12px;border:1px solid #EAD8C8;overflow:hidden;">
    ...
    <div style="background:#c0453a;padding:20px 24px;">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,0.8);margin-top:8px;font-weight:600;">User-reported inappropriate content</div>
    </div>
    ...
    ${coverPhoto ? `<div style="width:100%;overflow:hidden;background:#f0ebe3;"><img src="${coverPhoto}" alt="${spot.title}" style="width:100%;display:block;" /></div>` : ''}
    ...
    <div style="padding:24px;">
      ...
      const reportRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: FROM,
          to: [TO],
          subject: `User Report: ${reportType} — ${spot.title}`,
          html: reportHtml,
        }),
      })
...
<body style="margin:0;padding:0;background:#FDF8F0;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;">
  <div style="max-width:520px;margin:32px auto;background:#fff;border-radius:12px;border:1px solid #EAD8C8;overflow:hidden;">
    ...
    <div style="background:#fff3cd;border-bottom:3px solid #e6a817;padding:14px 24px;">
      <div style="font-size:13px;font-weight:700;color:#856404;margin-bottom:4px;">Auto-flagged by content moderation</div>
      <div style="font-size:12px;color:#856404;line-height:1.5;">This spot was automatically flagged for potential nudity or gore. It is <strong>not visible to the public</strong> until you approve it. Review the cover photo and approve or reject from the spot page.</div>
    </div>
    ` : ''}

    ${coverPhoto ? `
    <div style="width:100%;overflow:hidden;background:#f0ebe3;">
      <img src="${coverPhoto}" alt="${spot.title}" style="width:100%;display:block;" />
    </div>
    ` : ''}
    ...
    <div style="padding:24px;">
      ...
      <a href="${spotUrl}" style="display:block;text-align:center;background:${isPending ? '#c07820' : '#d4785a'};color:#fff;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:14px 24px;border-radius:8px;margin-bottom:16px;">${isPending ? 'Review Spot' : 'View Spot'}</a>

      <p style="margin:0;font-size:11px;color:#9a8878;text-align:center;line-height:1.5;">${isPending ? 'This spot is hidden from the public feed until you approve it.' : 'Spots are live instantly. Open the spot to edit or remove it if needed.'}</p>
    </div>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: FROM,
        to: [TO],
        subject: isPending ? `FLAGGED - REVIEW NEEDED: ${spot.title}` : `New spot added: ${spot.title}`,
        html,
      }),
    })
